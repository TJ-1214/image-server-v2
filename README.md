...
Once the runtime starts, you can access the project at [http://localhost:8080/image-server](http://localhost:8080/image-server).
...
```
./mvnw clean package
docker build -t image-server:v1 .
```

You can then run the Docker image by executing:

```
docker run -it --rm -p 8080:8080 image-server:v1
```
